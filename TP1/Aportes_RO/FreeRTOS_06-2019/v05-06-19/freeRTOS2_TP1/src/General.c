...
void Service(Module_Data_t *obj ){
	...
	//Frame_parameters.BufferAux = obj->MemoryAllocFunction( &mem_pool_big,0,0,sizeof(Data.Buffer),QmPoolOrMalloc);
	...
	if(XPointerQueUe != NULL){
		...
		//if (PcStringToSend == NULL) PcStringToSend = obj->MemoryAllocFunction( &mem_pool_big,0,0,obj->xMaxStringLength,QmPoolOrMalloc);
	...
	//obj-> MemoryFreeFunction(&mem_pool_big,Frame_parameters.BufferAux,0,NULL,QmPoolOrMalloc);
...
void Report( Module_Data_t *obj , char * XpointerQueue, uint8_t SelectHeapOrStack){
	...
	//if (PcStringToSend == NULL) PcStringToSend = obj->MemoryAllocFunction( &mem_pool_big,0,0,strlen(BuffA)+ NUM_ELEMENTOS_REST_FRAME,QmPoolOrMalloc);
	...
	//obj-> MemoryFreeFunction(&mem_pool_big,BSend,0,NULL,QmPoolOrMalloc);
